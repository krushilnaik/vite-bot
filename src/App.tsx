...
function App(props: Props) {
  ...
  //         // exchangeTokenAsync().then(function (token) {
  //         if (token) {
  //           directLine
  //             ?.postActivity({
  //               // @ts-ignore
  ...
  const init = async () => {
    const options: RequestInit = {
      ...
      headers: {
        Authorization:
          "Bearer 3JwG14bQwvaxbwY30UF3Of9kwTBO0B8Ct1VeQffCmT1siKxu5ZzVJQQJ99BBACL93NaAArohAAABAZBS2HHE.Eo9hUy2xV6tXBQRCCNIt52c4faEkm2VsTqpO7cplmazRjUWPPvyrJQQJ99BBACL93NaAArohAAABAZBS18nt",
      },
    };

    fetchJSON("https://directline.botframework.com/v3/directline/tokens/generate", options).then(({ token }) =>
      setToken(token)
    );
  };
